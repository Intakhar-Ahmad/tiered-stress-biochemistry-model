...
plt.rcParams.update({"font.size": 11, "axes.titlesize": 12, "axes.labelsize": 11, "xtick.labelsize": 10, "ytick.labelsize": 10, "legend.fontsize": 9})
...
def figure2_day7_heatmap(outdir: Path) -> None:
    ...
    ax.set_title("Day-7 model state across prespecified scenarios")
    cbar = fig.colorbar(im, ax=ax, fraction=0.025, pad=0.02)
    cbar.set_label("display preservation index")
...
def figure4_tier1(outdir: Path) -> None:
    ...
    for ax in axes[1]:
        ax.set_xlabel("time (h)"); ax.grid(alpha=0.25); ax.legend(fontsize=10)
...
def figure6_thresholds(outdir: Path) -> None:
    ...
    for y, val in zip(ypos, vals):
        if np.isnan(val):
            ax.text(2, y, "not crossed", va="center", fontsize=10)
        else:
            ax.text(val + 1, y, f"{val:.1f} h", va="center", fontsize=10)
...
def get_gui_spec():
    ...
    return TaskSpec(
        title='TSBM Corrected Figure Generator',
        description='Generates the revised, larger-font manuscript Figures 2–7 and the base CSV tables. Generated images appear in the gallery and can be saved as copies or opened in the basic editor.',
        runner=runner,
